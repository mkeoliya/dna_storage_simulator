...
def edit_distance(s1, s2, m, n, matrix, matrix2):
    ...
    if s1[m-1] == s2[n-1]:
        dist, ops = edit_distance(
            s1[:m-1], s2[:n-1], m-1, n-1, matrix, matrix2)
        ops = ops.copy()
        ops.append('=')
        matrix[m][n] = dist
        matrix2[m][n] = ops
        ...
        dist3, ops3 = edit_distance(
            s1[:m-1], s2[:n-1], m-1, n-1, matrix, matrix2)
        dist = min(dist1, dist2, dist3)
        distx = [dist1, dist2, dist3].index(dist) + 1  # index of min distance

        # if distances are equal, choose one at random
        if dist1 == dist2 and dist2 == dist3:
            distx = randint(1, 3)
        elif dist1 == dist2 and dist != dist3:
            distx = randint(1, 2)
        elif dist1 != dist2 and dist == dist3:
            distx = randint(2, 3)

        if dist == dist1 and distx == 1:
            ops1 = ops1.copy()
            ops1.append('+')
            matrix[m][n] = dist1 + 1
            matrix2[m][n] = ops1
