...
bases = ['A', 'C', 'G', 'T']
blank = '-'  # denotes gap in alignment
...
#        subprocess.run(["./clustalw2","-INFILE="+filename,"-OUTPUT=fasta", "-OUTFILE="+resname, "-PWDNAMATRIX=IUB", "-DNAMATRIX=IUB", "-TYPE=DNA", "-PWGAPOPEN=1.9", "-PWGAPEXT=1.9", "-GAPOPEN=1.9", "-GAPEXT=1.9"], check=True)
...
#    subprocess.run([myPath + "/dp" + str(strand_num), file_params, myPath], check=True)
#
#    s_aligned = []
#    if os.path.exists(resname):
#        with open(resname) as g:
...
#    f.close()
#    cmd = 'g++ -o3 '+ myPath+ '/dp' + str(strand_num) + '.cpp -o '+ myPath  +'/dp' + str(strand_num)
...
#        rev_mj = recover_strand_dp(rev_cluster, strand_length, file_params, myPath)
#
#    else:
#        mj = recover_strand_clustal(cluster, strand_length, file_params)
#        rev_mj = recover_strand_clustal(rev_cluster, strand_length, file_params)
