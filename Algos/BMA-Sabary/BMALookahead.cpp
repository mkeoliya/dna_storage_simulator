...
char BMAMajority(vector<string> &cluster, vector<int> &pointers)
{
    ...
    while (it != cluster.end())
    {
        ...
        {
            cerr << "darlin "
                 << " " << pointers[current_seq] << " " << seq[pointers[current_seq]] << endl;
...
char computeNextMajority(vector<string> &cluster, int positions[], int clus_size, int &position_lcs, string lcs)
{
    ...
    for (int i = 0; i < clus_size; i++)
    {
        if (cluster[i][positions[i]] == max_i)
        { // case of there was not "ERROR"
            positions[i]++;
        }
        else if (cluster[i][positions[i]] == next_c)
        { // case there was an "deletion"
            cluster[i].insert(positions[i]++, "_");
            //break;
        }
        else if (cluster[i][positions[i] + 1] == next_c)
        { // case there was an "substitution"
            //break;
        }
        else if (cluster[i][positions[i] + 1] == max_i)
        { // case there was an "insertion"
                                              ...
                                              const vector<vector<int> > &dp, vector<int> &priorities, mt19937 &generator)
{
    ...
    else
    {
        ...
        int subScore = subEqual * priorities[0], delScore = delEqual * priorities[2], insScore = insEqual * priorities[1];
        bool chooseSub = false, chooseDel = false, chooseIns = false;
        if (subScore > delScore and subScore > insScore)
        {
            chooseSub = true;
...
int main(int argc, char *argv[])
{
    ...
    string inputFileName = "evyat.txt";
    if (argc > 1)
    ...
    int testNum = getTestNum(inputFileName);
    try
    {
        ...
        for (d_prob = 0.01; d_prob <= 0.01; d_prob += 0.01)
        {
            ...
            strands_file.open(inputFileName);

            std::ofstream results_fail;
            results_fail.open(resultsFileName_fail);

            std::ofstream output;
            output.open(outputFileName);

            std::ofstream results_success;
            results_success.open(resultsFileName_success);
            if (!strands_file)
            {
                return -1;
            }
            ...
            while (!strands_file.eof())
            {
                ...
                string karin_w3 = LOOKAhead_Majority(cluster, original.length(), 3);
                string karin_rev_w3 = LOOKAhead_Majority(reverse_cluster, original.length(), 3);
